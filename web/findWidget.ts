...
class FindWidget {
	...
	constructor(view: GitGraphView) {
		...
		this.widgetElem.innerHTML = '<input id="findInput" type="text" placeholder="' + t('findWidget.title') + '" disabled/><span id="findCaseSensitive" class="findModifier" title="' + t('findWidget.caseSensitive') + '">Aa</span><span id="findRegex" class="findModifier" title="' + t('findWidget.regex') + '">.*</span><span id="findPosition"></span><span id="findPrev" title="' + t('findWidget.previousMatch') + '"></span><span id="findNext" title="' + t('findWidget.nextMatch') + '"></span><span id="findOpenCdv" title="' + t('findWidget.openCommitDetails') + '"></span><span id="findClose" title="' + t('findWidget.close') + ' (Escape)"></span>';
	...
	private updatePosition(position: number, scrollToCommit: boolean) {
		...
		this.positionElem.innerHTML = this.matches.length > 0 ? (this.position + 1) + t('findWidget.of') + this.matches.length : t('findWidget.noResults');
		this.view.saveState();
	}
